...
function To-WebRepoUrl([string]$remoteUrl) {
...
function Ensure-Gh {
  ...
  if (-not $gh) {
    Fail "gh CLI가 설치되어 있지 않습니다. 설치 후 다시 실행하세요."
  }
}
...
function Ensure-GhAuth {
  ...
  if ($LASTEXITCODE -ne 0) {
    Fail "gh 인증이 필요합니다. 먼저 'gh auth login'을 실행하세요."
  }
}
...
$repoUrl = To-WebRepoUrl (Get-RemoteHttpsUrl)
